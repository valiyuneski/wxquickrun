...
#include "res/checked_dis.xpm"
#include "res/unchecked_dis.xpm"

IMPLEMENT_CLASS(wxCheckedListCtrl, wxListCtrl)

BEGIN_EVENT_TABLE(wxCheckedListCtrl, wxListCtrl)
    EVT_LEFT_DCLICK(wxCheckedListCtrl::OnLeftDClick)
END_EVENT_TABLE()
...
{
	m_bCheckWhenSelecting = (bool)((style & wxCLC_CHECK_WHEN_SELECTING) >= 1);
	style = style & (~wxCLC_CHECK_WHEN_SELECTING);
	...
	m_imageList.Add(wxICON(IDI_ICON_PRI_BLOCKER));
	m_imageList.Add(wxICON(IDI_ICON_PRI_CRITICAL));
	m_imageList.Add(wxICON(IDI_ICON_PRI_MAJOR));
	m_imageList.Add(wxICON(IDI_ICON_PRI_MINOR));
	m_imageList.Add(wxICON(IDI_ICON_PRI_TRIVIAL));
	m_imageList.Add(wxICON(IDI_ICON_CLOCK));
...
void wxCheckedListCtrl::OnLeftDClick(wxMouseEvent& event)
{
	int flags;
